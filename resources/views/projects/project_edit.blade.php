...
<!-- main Content start -->
<div class="row">
	<?php  //echo '<pre>'; print_r($project_detail); echo '</pre>';?>
        ...
        <div class="col-md-6"> 
            ...
            <div class="form-group">
                <div class='input-group date' id='datetimepicker1'>
                    <input type='text' class="form-control datetimepicker" name="estimated_start_date" value="{{ $project_detail['estimated_start_date'] }}"/>
            ...
            <div class="form-group">
                <div class='input-group date' id='datetimepicker2'>
                    <input type='text' class="form-control datetimepicker" name="target_deadline" value="{{ $project_detail['target_deadline'] }}"/>
                    <span class="input-group-addon">
                        <span class="glyphicon glyphicon-calendar"></span>
                    </span>
                </div>
            ...
            <div class="form-group">
                                ...
                                <div class="col-sm-5">
                                    <div class="form-group">
                                        <select class="selectpicker" data-live-search="true" title="User" name="user_name[]">
                                            <option value="{{ $project_user->user_id }}" selected="">{{ $project_user->name }}</option>  
                                        </select>
                                    </div>
                                </div> 
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        <select class="selectpicker" data-live-search="true" title="Role" name="user_role[]">
                                            <option value="{{ $project_user->user_role }}" selected>
                                            @if ($project_user->user_role == 1) Manager 
                                            @elseif($project_user->user_role == 2) Team Leader @elseif($project_user->user_role == 3) Developer @endif
                                            </option>
                ...
                <div class="form-group">
                    ...
                    <select class="selectpicker" data-live-search="true" title="Change Status" name="status">
                        <option value="1" {{ $project_detail['status'] == 1 ? 'selected="selected"' : '' }}>Upcoming</option>
                        <option value="2" {{ $project_detail['status'] == 2 ? 'selected="selected"' : '' }}>Ongoing</option>
                        <option value="3" {{ $project_detail['status'] == 3 ? 'selected="selected"' : '' }}>Completed</option>
                        <option value="4" {{ $project_detail['status'] == 4 ? 'selected="selected"' : '' }}>Paused</option>
                    </select>
                </div>
